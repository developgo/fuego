...
func (m OrderedMap) EntrySet() Walker {
	return m.entries.(Walker)
}
...
func (m OrderedMap) KeySet() Walker {
	keySet := NewOrderedSet()
	for it := NewSetIterator(m.entries.(Walker)); it != nil; it = it.Forward() {
...
func (m OrderedMap) Insert(k Entry, v interface{}) MapMutator {
	return OrderedMap{
		entries: m.entries.
			Insert(MapEntry{
				K: k,
...
func (m OrderedMap) Delete(k Entry) MapMutator {
	return OrderedMap{
		entries: m.entries.
			Delete(MapEntry{
				K: k,
...
func (m OrderedMap) FirstRest() (Entry, interface{}, Map) {
	e, rest := m.entries.(Walker).FirstRest()
